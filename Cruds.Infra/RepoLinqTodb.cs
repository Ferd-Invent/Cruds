...
{
    public class RepoLinqTodb : IRepositoryCliente
    {
        public static RepoLinqTodb linqTodb;

        public ConnectionStringSettings ConnectionStringSettings1;

        public DbClientesDB dbClientes;

        public string connectionString = @"Server=localhost\SQLEXPRESS;Database=dbClientes;Trusted_Connection=True;";
        ...
        public void DeletarCliente(int codigo)
        {
            using (var db = new DbClientesDB())
            {
                ...
                //while(db.GetTable<Cliente>().Take(codigo).Delete()>0);
